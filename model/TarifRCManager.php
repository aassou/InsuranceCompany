...
class TarifRCManager{
	...
	public function update(TarifRC $tarifRC){
		...
		codeCompagnie=:codeCompagnie, codeUsage=:codeUsage, codeClasse=:codeClasse, codeSousClasse=:codeSousClasse, carburant=:carburant, puissanceFiscale=:puissanceFiscale, primeRC=:primeRC, majorationRemorque=:majorationRemorque, matiereInflamable=:matiereInflamable, transportPersonne=:transportPersonne, tauxCommission=:tauxCommission, tauxTPS=:tauxTPS, tauxTaxe=:tauxTaxe, timbre=:timbre, updated=:updated, updatedBy=:updatedBy
		WHERE id=:id')
		or die (print_r($this->_db->errorInfo()));
